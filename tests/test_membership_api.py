...
class TestMembershipAPIHubAdmins:
    ...
    @pytest.mark.asyncio
    async def test_add_lms_admins_as_lms_admin(self, membership_api, lms_admin_user, mock_backend):
    ...
    async def test_add_lms_admins_with_lms_add_enabled(
        self, membership_api_with_hub_add, lms_admin_user, mock_backend
    ):
        usernames = ["user1"]
        await membership_api_with_hub_add.add_lms_admins(lms_admin_user, usernames)

        mock_backend.ensure_users_exist.assert_called_once_with(usernames, create_if_missing=True)

    @pytest.mark.asyncio
    async def test_add_lms_admins_permission_denied(
        self, membership_api, math101_2024ws_student_user
    ):
        with pytest.raises(APIPermissionError):
            await membership_api.add_lms_admins(math101_2024ws_student_user, ["user1"])

    @pytest.mark.asyncio
    async def test_remove_lms_admins_as_lms_admin(
    ...
    @pytest.mark.asyncio
    async def test_remove_lms_admins_permission_denied(
        self, membership_api, math101_2024ws_student_user
    ):
        with pytest.raises(APIPermissionError):
            await membership_api.remove_lms_admins(math101_2024ws_student_user, ["user1"])

    @pytest.mark.asyncio
    async def test_list_lms_admins_as_lms_admin(self, membership_api, lms_admin_user, mock_backend):
    ...
    @pytest.mark.asyncio
    async def test_list_lms_admins_permission_denied(
        self, membership_api, math101_2024ws_student_user
    ):
        with pytest.raises(APIPermissionError):
            await membership_api.list_lms_admins(math101_2024ws_student_user)
...
class TestMembershipAPICourseCreators:
    ...
    @pytest.mark.asyncio
    async def test_remove_course_creators_as_lms_admin(
        self, membership_api, lms_admin_user, mock_backend
    ):
    ...
    @pytest.mark.asyncio
    async def test_list_course_creators_as_lms_admin(
        self, membership_api, lms_admin_user, mock_backend
    ):
        expected = ["creator1"]
        mock_backend.get_group_members.return_value = expected

        result = await membership_api.list_course_creators(lms_admin_user)
...
class TestMembershipAPICourseOwners:
    ...
    @pytest.mark.asyncio
    async def test_remove_course_owners(self, membership_api, lms_admin_user, mock_backend):
        course_id = "math101"
    ...
    @pytest.mark.asyncio
    async def test_list_course_owners(self, membership_api, lms_admin_user, mock_backend):
        course_id = "math101"
        expected = ["owner1"]
        mock_backend.get_group_members.return_value = expected
...
class TestMembershipAPIInstructors:
    ...
    async def test_add_instructors_as_course_owner(self, membership_api, mock_backend):
        ...
        expected_group = "lms.course.math101.term.2024ws.instructor"
        mock_backend.ensure_group_exists.assert_called_once_with(
            expected_group, create_if_missing=True
        )
        mock_backend.add_users_to_group.assert_called_once_with(expected_group, usernames)
    ...
    async def test_add_instructors_as_instructor(self, membership_api, mock_backend):
        instructor = UserStub(username="inst", groups=["lms.course.math101.term.2024ws.instructor"])
    ...
    @pytest.mark.asyncio
    async def test_remove_instructors(self, membership_api, lms_admin_user, mock_backend):
        mock_backend.get_group_members.return_value = ["instructor1", "instructor2"]

        await membership_api.remove_instructors(
            lms_admin_user, "math101", "2024ws", ["instructor1"]
        )

        mock_backend.remove_users_from_group.assert_called_once_with(
            "lms.course.math101.term.2024ws.instructor", ["instructor1"]
        )

    @pytest.mark.asyncio
    async def test_list_instructors(self, membership_api, lms_admin_user, mock_backend):
        expected = ["instructor1"]
        mock_backend.get_group_members.return_value = expected

        result = await membership_api.list_instructors(lms_admin_user, "math101", "2024ws")
...
class TestMembershipAPITeachingAssistants:
    ...
    @pytest.mark.asyncio
    async def test_add_teaching_assistants(self, membership_api, lms_admin_user, mock_backend):
        usernames = ["ta1"]
    ...
    @pytest.mark.asyncio
    async def test_remove_teaching_assistants(self, membership_api, lms_admin_user, mock_backend):
        mock_backend.get_group_members.return_value = ["ta1", "ta2"]

        await membership_api.remove_teaching_assistants(
            lms_admin_user, "math101", "2024ws", ["ta1"]
        )
    ...
    @pytest.mark.asyncio
    async def test_list_teaching_assistants(self, membership_api, lms_admin_user, mock_backend):
        expected = ["ta1"]
        mock_backend.get_group_members.return_value = expected

        result = await membership_api.list_teaching_assistants(lms_admin_user, "math101", "2024ws")
...
class TestMembershipAPIObservers:
    ...
    @pytest.mark.asyncio
    async def test_add_observers(self, membership_api, lms_admin_user, mock_backend):
        usernames = ["observer1"]
        await membership_api.add_observers(lms_admin_user, "math101", "2024ws", usernames)

        expected_group = "lms.course.math101.term.2024ws.observer"
        mock_backend.ensure_group_exists.assert_called_once_with(
            expected_group, create_if_missing=True
        )
        mock_backend.add_users_to_group.assert_called_once_with(expected_group, usernames)
    ...
    @pytest.mark.asyncio
    async def test_remove_observers(self, membership_api, lms_admin_user, mock_backend):
        mock_backend.get_group_members.return_value = ["observer1"]

        await membership_api.remove_observers(lms_admin_user, "math101", "2024ws", ["observer1"])

        mock_backend.remove_users_from_group.assert_called_once()

    @pytest.mark.asyncio
    async def test_list_observers(self, membership_api, lms_admin_user, mock_backend):
        expected = ["observer1"]
        mock_backend.get_group_members.return_value = expected

        result = await membership_api.list_observers(lms_admin_user, "math101", "2024ws")
...
class TestMembershipAPIStudents:
    ...
    async def test_add_students_as_instructor(self, membership_api, mock_backend):
        ...
        expected_group = "lms.course.math101.term.2024ws.student"
        mock_backend.ensure_group_exists.assert_called_once_with(
            expected_group, create_if_missing=True
        )
        mock_backend.add_users_to_group.assert_called_once_with(expected_group, usernames)
    ...
    @pytest.mark.asyncio
    async def test_remove_students(self, membership_api, lms_admin_user, mock_backend):
        mock_backend.get_group_members.return_value = ["student1", "student2"]

        await membership_api.remove_students(lms_admin_user, "math101", "2024ws", ["student1"])

        mock_backend.remove_users_from_group.assert_called_once_with(
            "lms.course.math101.term.2024ws.student", ["student1"]
        )

    @pytest.mark.asyncio
    async def test_list_students_as_observer(self, membership_api, mock_backend):
        observer = UserStub(username="obs", groups=["lms.course.math101.term.2024ws.observer"])
    ...
    async def test_list_students_permission_denied(
        ...
        different_course_student = UserStub(
            username="alice", groups=["lms.course.phys101.term.2024ws.student"]
        )
...
class TestMembershipAPIEdgeCases:
    ...
    @pytest.mark.asyncio
    async def test_add_empty_list_of_users(self, membership_api, lms_admin_user, mock_backend):
        await membership_api.add_lms_admins(lms_admin_user, [])
    ...
    async def test_hub_admin_has_all_permissions(
        ...
        await membership_api.add_lms_admins(lms_admin_user, ["user1"])
        await membership_api.add_course_owners(lms_admin_user, "course1", ["user2"])
        await membership_api.add_students(lms_admin_user, "course1", "term1", ["user3"])
